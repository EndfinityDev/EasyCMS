...
{
    static string GetGameCarsFolder(in string executablePath)
    {
        string executableDir = Path.GetDirectoryName(executablePath);
        string carsFolder = Path.Combine(executableDir, "Car Mechanic Simulator 2021_Data/StreamingAssets/Cars");
        return carsFolder;
    }

    public static void CopyCarAssetsToGame(in string carPath, in string executablePath)
    {
        bool error = false;
        ...
        {
            Debug.LogError($"[AssetCopying.CopyCarAssetsToGame] | Failed to delete meta files. Error: {ex.Message}; Path: \"{targetPath}\"");
            error = true;
        }
    ...
    static void DeleteMetaFiles(in string directory)
    {
    ...
    public static void CopyFileFromGame(in string fileName, in string carPath, in string executablePath)
    {
        if (executablePath == null || executablePath == string.Empty)
        {
            EditorUtility.DisplayDialog("Missing Executable Path", "Executable path is not defined. Please define it in the inspector window before running this command", "Ok");
        ...
        {
            EditorUtility.DisplayDialog("Failed to find car files", $"Could not find file \"{fileName}\" for car \"{carName}\"", "Ok");
            return;
        }
        ...
        {
            Debug.LogError($"[AssetCopying.CopyFileFromGame] | Failed to copy file. Error: {ex.Message}; File: \"{targetFilePath}\"; \"{localFilePath}\"");
            error = true;
        }
        if (error)
        {
    ...
    public static bool CheckIfGameFileNewer(in string fileName, in string carPath, in string executablePath)
    {
        string[] splitCarPath = carPath.Split('/');
        splitCarPath[splitCarPath.Length - 1] = splitCarPath[splitCarPath.Length - 1].Replace(" ", "_");
        string carName = splitCarPath[splitCarPath.Length - 1];
