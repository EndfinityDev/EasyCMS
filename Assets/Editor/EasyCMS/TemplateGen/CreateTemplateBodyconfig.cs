...
public class CreateTemplateBodyconfig
{
    ...
    public static void ProcessCarFolder(in string carFolderPath)
    {
        ...
        {
            if (s_UnmountTable.TryGetValue(meshName, out string mountToMesh))
            {
        ...
        {
            outConfigString += $"{pair.Key}={string.Join(",", pair.Value)}\r\n";
        }
